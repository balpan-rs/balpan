    ...
    T: Copy + PartialEq + Into<usize>,
{
    ...
    fn next(&mut self) -> Option<usize> {
        find_from_position(&self.searcher, self.text, self.pos)
            .and_then(|pos| {
...
/// assert_eq!(2, result);
/// ```
pub fn find_pending_character_index(chars: &Vec<char>, start: usize, pattern: &char) -> usize {
...
pub fn get_suffix_table<T: PartialEq>(pattern: &[T]) -> Vec<usize> {
    ...
    for suffix_len in 1 .. pattern.len() {
        ...
        for i in (0 .. len - suffix_len).rev() {
            ...
            if suffixes[i + suffix_len - 1] == suffix_len - 1 && pattern[i] == pattern[len - suffix_len] {
...
///     A  B  A  B
///     3  3  2  1
pub fn build_suffixes_table<T: PartialEq>(needle: &[T]) -> Vec<usize> {
...
pub fn find_from_position<'a, T, U>(pattern: &'a U, text: &'a [T], mut position: usize) -> Option<usize> 
where
